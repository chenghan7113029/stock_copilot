...
- `aaa_corporate_yield = 5.30`（%）
- `discount_rate = 10.0`（%，简化折现率）
- `tax_rate = 25.0`（%）
...
#### Scenario: 兜底使用硬编码默认值
- **WHEN** `StockData.cost_of_capital = None` 且 `config` 中无对应配置
- **THEN** `AssumptionProvider.get_discount_rate(stock_data)` 返回 `10.0`
