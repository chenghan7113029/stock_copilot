...
**真正还没 apply 的**，目前是：T-1 MOS 按原型阈值、价值 V2 专用方法（分 Phase）、飞书常看推送、K 线形态、布林带。
...
1. **`add-mos-thresholds-by-prototype`（T-1）** — 小而独立，可先做  
2. **`add-value-v2-prototype-methods` P1** — 比亚迪浅情景（持仓优先）  
3. **`add-feishu-watchlist-push`** — 通勤交付  
4. 形态 / 布林带 — 可穿插
