...
class DCF(BaseValuation):
    ...
    best_for = [
        "Growth companies with predictable FCF",
        "Mature businesses",
    ...
    def _resolve_fcf(self, stock) -> tuple[float | None, list[str]]:
        warnings: list[str] = []
        fcf = stock.fcf
        if fcf is not None:
            return fcf, warnings
        ...
        if ocf is not None and capex is not None:
            ...
            warnings.append("FCF derived from operating_cash_flow - abs(capex)")
            return derived, warnings
        return None, warnings

    def calculate(self, stock) -> ValuationResult:
        fcf, derive_warnings = self._resolve_fcf(stock)
        if fcf is None:
            return self._create_error_result(
                stock,
                "Missing required data: fcf",
                ["fcf"],
            )

        is_valid, missing, warnings = self.validate_data(stock)
        warnings = derive_warnings + warnings
        if not is_valid:
            return self._create_error_result(
        ...
        g_term_val = (
            self.terminal_growth if self.terminal_growth is not None else getattr(stock, "terminal_growth", None)
        )
        ...
        value_low = self._run_dcf_sensitivity(
            stock, fcf, shares, net_debt, g1 - 0.02, g2 - 0.01, g_term - 0.005, r + 0.02
        )
        value_high = self._run_dcf_sensitivity(
            stock, fcf, shares, net_debt, g1 + 0.02, g2 + 0.01, g_term + 0.005, r - 0.02
        )
        ...
        if tv_pct > 60:
            analysis.append(
                "Warning: Terminal value is >60% of total - consider sensitivity to growth assumptions"
            )
        if warnings:
    ...
    def _run_dcf_sensitivity(
        self, stock, fcf, shares, net_debt, g1, g2, g_term, r
    ) -> float:
...
class ReverseDCF(BaseValuation):
    ...
    def calculate(self, stock) -> ValuationResult:
        ...
        if fcf is None:
            return self._create_error_result(stock, "Missing required data: fcf", ["fcf"])
        if fcf <= 0:
        ...
        if not converged:
            analysis.append(
                f"Warning: Calculation did not fully converge after {self.MAX_ITERATIONS} iterations"
            )

        if mid < 0:
            analysis.append(
                "Market expects negative growth - potential value trap or distressed situation"
            )
        elif mid > 30:
            analysis.append(f"High implied growth ({mid:.1f}%) - verify if sustainable")
        elif mid < 5:
            analysis.append(
                f"Low implied growth ({mid:.1f}%) - mature or declining business expected"
            )
        ...
        growth_high = mid + 5
        analysis.append(
            f"Sensitivity range: {max(0, growth_low):.1f}% to {min(50, growth_high):.1f}% growth"
        )
        ...
        return ValuationResult(
            ...
            details={
                "implied_growth_rate": round(mid, 1),
            ...
            fair_value_range=(
                ValuationRange(
                    low=self._price_at_growth(stock, max(growth_low, 0) / 100, g_term, r, net_debt),
                    base=current_price,
                    high=self._price_at_growth(stock, min(growth_high, 50) / 100, g_term, r, net_debt),
                )
                if converged
                else None
            ),
    ...
    def _calculate_ev(self, fcf, g1, g2, g_term, r) -> float:
        projected_fcf = fcf
        total_pv = 0.0
    ...
    def _price_at_growth(self, stock, g1, g_term, r, net_debt: float) -> float:
        g2 = g1 * 0.5
        implied_ev = self._calculate_ev(stock.fcf, g1, g2, g_term, r)
        implied_equity = implied_ev - net_debt
        return implied_equity / stock.shares_outstanding if implied_equity > 0 else 0.0
