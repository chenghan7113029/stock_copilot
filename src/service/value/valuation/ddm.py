...
class DDM(BaseValuation):
    ...
    def calculate(self, stock) -> ValuationResult:
        ...
        if dividend <= 0:
            return self._create_error_result(
                stock, "Dividend must be positive for DDM", ["dividend_per_share"]
            )
        ...
        iv_low = (
            next_dividend / (r + 0.02 - max(g - 0.02, 0))
            if r + 0.02 > max(g - 0.02, 0)
            else 0
        )
        ...
        if payout_ratio > 80:
            analysis.append(
                f"Warning: High payout ratio ({payout_ratio:.0f}%) - dividend growth may be limited"
            )
        ...
        confidence = (
            "High"
            if 0 < g < 0.08 and payout_ratio < 70
            else ("Medium" if g < 0.12 else "Low")
        )
...
class TwoStageDDM(BaseValuation):
    ...
    def calculate(self, stock) -> ValuationResult:
        ...
        if current_dividend <= 0:
            return self._create_error_result(
                stock, "Dividend must be positive", ["dividend_per_share"]
            )
        ...
        iv_low = self._calc_two_stage(current_dividend, g1 - 0.02, g2 - 0.01, r + 0.02)
        iv_high = self._calc_two_stage(current_dividend, g1 + 0.02, g2 + 0.01, r - 0.02)
    ...
    def _calc_two_stage(self, current_div: float, g1: float, g2: float, r: float) -> float:
        if r <= g2 or g1 <= -1:
            return 0.0
