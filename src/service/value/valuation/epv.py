...
class EPV(BaseValuation):
    ...
    def calculate(self, stock) -> ValuationResult:
        ...
        operating_margin_pct = getattr(stock, "operating_margin", None)
        if operating_margin_pct is None:
            ...
            if ebit is not None and revenue is not None and revenue > 0:
                ...
                warnings.append("Operating margin derived from EBIT/revenue")
            else:
                return self._create_error_result(
                    stock,
                    "Missing required data: operating_margin",
                    ["operating_margin"],
                )

        operating_margin = operating_margin_pct / 100
        tax_rate = stock.tax_rate / 100
        depreciation = stock.depreciation if stock.depreciation is not None else 0.0
        capex = stock.capex if stock.capex is not None else 0.0
        cost_of_capital = (
            self.cost_of_capital
            if self.cost_of_capital is not None
            else stock.cost_of_capital
        ) / 100
        ...
        if operating_margin <= 0:
            return self._create_error_result(
                stock,
                f"Operating margin must be positive (got {operating_margin * 100:.1f}%)",
                [],
            )
        ...
        if cost_of_capital <= 0:
            return self._create_error_result(
                stock,
                f"Cost of capital must be positive (got {cost_of_capital * 100:.1f}%)",
                [],
            )

        ebit = revenue * operating_margin
        nopat = ebit * (1 - tax_rate)
        maintenance_capex = abs(capex) * maintenance_pct if capex != 0 else 0.0
        ...
        growth_priced_in = (
            ((stock.current_price / epv_per_share) - 1) * 100 if epv_per_share > 0 else 0
        )
        ...
        analysis = [
            "Value assuming ZERO future growth",
        ...
        if growth_priced_in > 50:
            ...
            analysis.append("Trading below zero-growth value - potential value opportunity")
        ...
        confidence = (
            "High"
            if 0 < growth_priced_in < 30
            else ("Medium" if 0 < growth_priced_in < 50 else "Low")
        )

        applicability = "Applicable"
        if epv_per_share <= 0:
            applicability = "Limited"
            analysis.append("Equity value below zero after net debt adjustment")
