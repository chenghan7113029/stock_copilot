...
class GrahamNumber(BaseValuation):
    ...
    def calculate(self, stock) -> ValuationResult:
        ...
        if bvps < self.BVPS_THRESHOLD:
            return self._create_error_result(
                stock,
                f"Graham Number not applicable: BVPS ({bvps:.2f}) is below "
                f"{self.BVPS_THRESHOLD:.0f} threshold.",
                ["bvps"],
            )
        ...
        return ValuationResult(
            ...
            fair_value_range=ValuationRange(
                low=round(graham_low, 2),
                base=round(graham_number, 2),
                high=round(graham_high, 2),
            ),
...
class GrahamFormula(BaseValuation):
    ...
    def calculate(self, stock) -> ValuationResult:
        ...
        if growth_rate < self.MIN_GROWTH:
            ...
            warnings.append(
                f"Growth rate {original_growth}% capped to {self.MIN_GROWTH}% (minimum)"
            )
        elif growth_rate > self.MAX_GROWTH:
            growth_rate = self.MAX_GROWTH
            warnings.append(
                f"Growth rate {original_growth}% capped to {self.MAX_GROWTH}% (Graham's max)"
            )
        ...
        analysis = [
            "Original Graham formula: V = (EPS × (8.5 + 2g) × 4.4) / Y",
...
class NCAV(BaseValuation):
    ...
    def calculate(self, stock) -> ValuationResult:
        ...
        if ncav_total <= 0:
            ...
            analysis.append(
                f"*** Net-Net opportunity! Price below {self.safety_margin * 100:.0f}% of NCAV ***"
            )
            analysis.append(f"Buy Target: {buy_target:.2f} (2/3 of NCAV)")
        elif stock.current_price < ncav_per_share:
            analysis.append(
                f"Below full NCAV but above {self.safety_margin * 100:.0f}% safety margin"
            )
            analysis.append(
                f"Margin of safety: "
                f"{((ncav_per_share - stock.current_price) / ncav_per_share * 100):.1f}%"
            )
        else:
            analysis.append("Price above NCAV - not a cigar butt opportunity")
            analysis.append(
                f"Premium to NCAV: "
                f"{((stock.current_price - ncav_per_share) / ncav_per_share * 100):.1f}%"
            )
        ...
        return ValuationResult(
            ...
            confidence="Medium",
            fair_value_range=ValuationRange(
                low=round(ncav_liquidating_per_share, 2),
                base=round(ncav_per_share, 2),
                high=round(ncav_per_share, 2),
            ),
