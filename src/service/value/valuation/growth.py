...
class PEG(BaseValuation):
    ...
    def calculate(self, stock) -> ValuationResult:
        ...
        if growth_rate <= 0:
            return self._create_error_result(
                stock, f"Growth rate must be positive (got {growth_rate:.1f}%)", []
            )
        ...
        premium_discount = (
            ((fair_price - stock.current_price) / stock.current_price) * 100 if fair_price > 0 else 0
        )
...
class GARP(BaseValuation):
    ...
    def calculate(self, stock) -> ValuationResult:
        ...
        if g <= 0:
            return self._create_error_result(stock, "Growth rate must be positive for GARP", [])
        ...
        pv_low = (
            eps * ((1 + g * 0.8) ** self.years) * self.target_pe * 0.9 / ((1 + r) ** self.years)
        )
        pv_high = (
            eps * ((1 + g * 1.2) ** self.years) * self.target_pe * 1.1 / ((1 + r) ** self.years)
        )
...
class EVEBITDA(BaseValuation):
    ...
    def calculate(self, stock) -> ValuationResult:
        ...
        if ebitda is None or ebitda <= 0:
            ...
            if ebit is not None and ebit > 0 and depreciation is not None and depreciation > 0:
                ebitda = ebit + depreciation
                warnings.append(
                    f"EBITDA calculated from EBIT ({ebit / 1e9:.2f}B) + Depreciation ({depreciation / 1e9:.2f}B)"
                )
                ...
                elif stock.net_income is not None and stock.net_income > 0:
        ...
        if self.fair_multiple is not None:
            ...
            fair_multiple = getattr(stock, "ev_ebitda_multiple", None)
            if fair_multiple is None:
                fair_multiple = self.INDUSTRY_MULTIPLES["default"]

            roe = stock.roe if stock.roe is not None else 0.0
            growth_rate = stock.growth_rate if stock.growth_rate is not None else 0.0
            if roe > 20:
                fair_multiple *= 1.2
            if growth_rate > 15:
                fair_multiple *= 1.1
            if ev > 0 and net_debt > ev * 0.5:
                fair_multiple *= 0.85
        ...
        analysis = [
            ...
            (
                ...
                f"(Market Cap: {market_cap / 1e9:.2f}B + Net Debt: {net_debt / 1e9:.2f}B)"
            ),
        ...
        if stock.pe_ratio > 0:
            ...
            if current_ev_ebitda < stock.pe_ratio * 0.7:
                analysis.append(
                    "Note: EV/EBITDA significantly lower than P/E - check for high depreciation or debt"
                )
        ...
        confidence = (
            "High"
            if stock.ebitda is not None and stock.ebitda > 0 and len(warnings) == 0
            else ("Medium" if len(warnings) <= 2 else "Low")
        )
