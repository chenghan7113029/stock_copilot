...
class MagicFormula(BaseValuation):
    ...
    not_for = [
        ...
        "Negative earnings",
        "Asset-light businesses with negative working capital",
    ]
    ...
    def calculate(self, stock) -> ValuationResult:
        ...
        if (ebit is None or ebit <= 0) and operating_margin > 0 and revenue is not None and revenue > 0:
        ...
        net_debt = stock.net_debt if stock.net_debt is not None else 0.0
        ...
        nfa = stock.net_fixed_assets if stock.net_fixed_assets is not None else 0.0
        nwc = stock.net_working_capital if stock.net_working_capital is not None else 0.0
        invested_capital = nfa + nwc

        if invested_capital <= 0:
            bvps = stock.bvps if stock.bvps is not None else 0.0
            shares = stock.shares_outstanding
            total_equity = bvps * shares if bvps > 0 and shares > 0 else 0.0
            short_term = stock.short_term_debt if stock.short_term_debt is not None else 0.0
            long_term = stock.long_term_debt if stock.long_term_debt is not None else 0.0
            total_debt = net_debt if net_debt > 0 else (short_term + long_term)
            fallback_ic = total_equity + total_debt
            if fallback_ic > 0:
                invested_capital = fallback_ic
                warnings.append("Invested capital estimated from equity + debt fallback")
            else:
                return self._create_error_result(
                    stock,
                    f"Invalid Invested Capital: Net Fixed Assets ({nfa / 1e9:.2f}B) + "
                    f"NWC ({nwc / 1e9:.2f}B) = {invested_capital / 1e9:.2f}B, "
        ...
        fair_price = (
            fair_equity / stock.shares_outstanding
            if stock.shares_outstanding > 0 and fair_equity > 0
            else 0
        )
        ...
        if fair_price <= 0:
            return ValuationResult(
                ...
                analysis=[
                    ...
                    f"Return on Capital: {return_on_capital:.1f}%"
                    if invested_capital > 0
                    else "ROC: N/A (negative invested capital)",
        ...
        if ey_pass and roc_pass:
            ...
            quality_analysis = (
                f"Good earnings yield but ROC ({return_on_capital:.1f}%) below benchmark"
            )
        elif roc_pass:
            quality = "Good Quality but Expensive"
            quality_analysis = (
                f"Good ROC but earnings yield ({earnings_yield:.1f}%) below requirement"
            )
        else:
            quality = "Below Thresholds"
            quality_analysis = "Below both EY and ROC thresholds - not a Magic Formula candidate"

        fair_price_low = (ebit / (self.required_ey / 100 + 0.03) - net_debt) / stock.shares_outstanding
        fair_price_high = (ebit / (self.required_ey / 100 - 0.03) - net_debt) / stock.shares_outstanding

        ey_status = "PASS" if ey_pass else "FAIL"
        roc_status = "PASS" if roc_pass else "FAIL"
        analysis = [
            f"Earnings Yield (EBIT/EV): {earnings_yield:.1f}% "
            f"(Required: {self.required_ey}%) - {ey_status}",
            (
                f"Return on Capital: {return_on_capital:.1f}% "
                f"(Benchmark: {self.benchmark_roc}%) - {roc_status}"
                if invested_capital > 0
                else "ROC: N/A"
            ),
